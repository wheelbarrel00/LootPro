...
- Alert on value box on the Alerts tab. Set a gold amount and the rare-drop alert (flash, sound, and color) also fires when a single drop is worth at least that much, on top of the quality and notable triggers. Set it to 0 to turn it off.
...
### Bug Fixes
- **Session Recap now resets when you log out** — it decided whether a load was a `/reload` from a game flag that misfires on a normal login on 12.0, so a session could survive logging out, switching characters, and even restarting, and keep counting for days until you used Reset Session. A `/reload` still preserves the current session; logging out, switching characters, or restarting now starts a fresh one.
...
### Maintenance
- Code comment cleanup across the add-on — verbose comments trimmed to clean, hand-written one-liners — plus a TOC version bump. No functional or behavioral changes.
...
### Improvements
- **Multi-version interface support** — `LootPro.toc` now declares `Interface: 120001, 120005, 120007` so the addon loads cleanly across all current retail patch builds without an out-of-date warning.

## [2.4.3] - 2026-05-04

### Added
- **Quick Loot toggle** — new option in the Custom tab lets users enable or disable Quick Loot.
